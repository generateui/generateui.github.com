...
_WindowEventsImpl.prototype.get$hashChange = function() {
  return this._get("hashchange");
}
...
function Drop() {
  var $this = this;
  get$$window().get$on().get$hashChange().add($wrap_call$1((function (e) {
    return $this.loadFromHash();
  })
  ), false);
  this.loadFromHash();
}
Drop.prototype.loadFromHash = function() {
  if (get$$window().location.hash == "") {
    this.showPage("welcome");
  }
  else {
    var splitted = get$$window().location.hash.split$_("/");
    var section = splitted.$index((0)).substring$1((1));
    if (splitted.get$length() == (2)) {
      this.showPage(("" + section + "/" + splitted.$index((1))));
    }
    else {
...
Drop.prototype.showPage = function(relativeUri) {
  var $this = this;
  var req = _XMLHttpRequestFactoryProvider.XMLHttpRequest$get$factory(("" + relativeUri + ".html"), $wrap_call$1((function (oh) {
    print$(oh.get$status());
    get$$document().query("#content").set$innerHTML(oh.get$responseText());
  })
  ));
  req.get$on().get$error().add($wrap_call$1((function (er) {
    return $this.showError();
  })
  ), false);
}
Drop.prototype.showError = function() {
  var error = get$$document().query("#error");
  error.get$style().set$display("block");
  get$$document().query("#content").set$innerHTML(error.get$innerHTML());
}
function main() {
  new Drop();
}
